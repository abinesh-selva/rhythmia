...
export function LanguageDetailView({ language, tracks }: { language: Language; tracks: TrackItem[] }) {
  ...
  return (
    <div className="flex flex-col min-h-full pb-20" onClick={() => setActiveMenuTrackId(null)}>
      <div className="relative p-6 md:p-10 flex flex-col md:flex-row md:items-end gap-6 border-b border-cream/5 overflow-hidden bg-gradient-to-b from-green/30 to-forest-dark">
      ...
      <div className="px-6 md:px-10">
        {activeTracks.length === 0 ? (
          <div className="p-12 text-center text-muted text-sm border border-cream/5 rounded-2xl bg-panel/30">No tracks in this language yet.</div>
        ) : (
