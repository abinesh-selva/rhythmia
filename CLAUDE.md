...
Background auto-sync fires on app load with a 1-hour localStorage gate (`soniqo_last_sync_time`).
...
Every Supabase operation in `AudioContext` has an `else` branch writing to localStorage under `soniqo_local_*` keys. The offline path is triggered automatically when `isSupabaseConfigured` is false. All features work identically offline except Google OAuth.
